...
def compute_grouping_accuracy(df_groundtruth,
                              df_parsedlog,
    ...
    (GA, FGA) = get_accuracy_ga(df_groundtruth['EventTemplate'],
                                df_parsedlog['EventTemplate'], filter_templates)
    print('Grouping_Accuracy (GA): %.4f, FGA: %.4f,' % (GA, FGA))
    return GA, FGA


def get_accuracy_ga(series_groundtruth,
                    series_parsedlog,
                    filter_templates=None):
    """ Compute accuracy metrics between log parsing results_author_2k and ground truth
    ...
    for ground_truthId, group in grouped_df:
        ...
        if series_parsedlog_logId_valuecounts.size == 1:
            ...
            if len(group) == series_parsedlog[series_parsedlog ==
                                              parsed_eventId].size:
                if (filter_templates is None) or (ground_truthId
                                                  in filter_templates):
    ...
    if filter_templates is not None:
        GA = float(accurate_events) / len(
            series_groundtruth[series_groundtruth.isin(filter_templates)])

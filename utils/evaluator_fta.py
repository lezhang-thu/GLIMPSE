...
def compute_template_level_accuracy(dataset,
                                    df_groundtruth,
                                    df_parsedresult,
    ...
    null_logids = df_groundtruth[~df_groundtruth['EventTemplate'].isnull(
    )].index
    ...
    df_combined = pd.concat([series_groundtruth, series_parsedlog],
                            axis=1,
                            keys=['groundtruth', 'parsedlog'])
    ...
    for identified_template, group in grouped_df:
        ...
        if filter_templates is not None and len(
                corr_oracle_templates.intersection(set(filter_templates))) > 0:
        ...
        if corr_oracle_templates == {identified_template}:
            if (filter_templates is None) or (identified_template
                                              in filter_templates):
                correct_parsing_templates += 1
        else:
            wrong_idx += 1
            print(
                f"{wrong_idx}-num-{len(group)}, {corr_oracle_templates} | {set([identified_template])}"
            )
    ...
    t1 = len(grouped_df) if filter_templates is None else len(
        filter_identify_templates)
    t2 = len(series_groundtruth_valuecounts
            ) if filter_templates is None else len(filter_templates)
    print("Identify : {}, Groundtruth : {}".format(t1, t2))
    return t1, t2, FTA, PTA, RTA


# def compute_template_level_accuracy_lstm(dataset, df_groundtruth, df_parsedresult, filter_templates=None):
#     """
#     Conduct the template-level analysis using 4-type classifications
#
#     :param dataset:
...
#     :param comparison_results_df: template analysis results_author_2k (dataFrame)

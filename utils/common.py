...
datasets = [
    ...
    'Thunderbird', 'Windows', 'Linux', 'Mac', 'Android', 'HealthApp', 'Apache',
    'OpenSSH', 'Proxifier'
...
def get_pattern_from_template(template):
    ...
    return "^" + spaced_escape.replace(
        r"<\*>", r"(\S+?)") + "$"  # a single <*> can consume multiple tokens
...
def common_args():
    ...
    parser.add_argument('--example_size', type=int, default=3)
    parser.add_argument('--model', type=str, default="gpt-3.5-turbo-0613")
    parser.add_argument('--zsl', type=str, default="")
    parser.add_argument('--example_dataset',
                        type=str,
                        default="same",
                        choices=[
                            "same",
    ...
    parser.add_argument('--selection_method', type=str, default="LILAC")
...
def unique_output_dir(name):
    return os.path.join(
        '{}_result'.format(name),
        '{}_{}'.format(datetime.now().strftime("%Y%m%d_%H%M%S"), os.getpid()))
...
def correct_templates_and_update_files(dir_path,
                                       log_file_basename,
                                       inplace=False):
    """
    ...
    org_structured_log_file = os.path.join(
        dir_path, log_file_basename + '_structured.csv')
    ...
    templates_df = structured_logs_df.drop_duplicates(
        subset='EventTemplate').dropna(subset=['EventTemplate'])

    # convert templates_df into dict (key: EventId, value: EventTemplate)
    templates_dict = templates_df.set_index(
        'EventId')['EventTemplate'].to_dict()
    ...
    for index, row in structured_logs_df.iterrows():
        ...
        if is_matched is False:
            print('*** WARN: No matching template; EventId:', row['EventId'],
                  'message:', row['Content'])
    ...
    if inplace:  # used for tool-generated structured log file
        ...
        structured_logs_df.to_csv(os.path.join(
            dir_path, log_file_basename + '_structured_corrected.csv'),
                                  index=False)
    ...
    new_templates_df = pd.DataFrame(new_templates,
                                    columns=['EventId', 'EventTemplate'])
    new_templates_df.to_csv(os.path.join(
        dir_path, log_file_basename + '_templates_corrected.csv'),
                            index=False)

    print('Structured log and templates file update done. [Time taken: {:.3f}]'.
...
def correct_templates(templates_dict):
    ...
    start_time = time.time()
    for tid, template in sorted(
            templates_dict.items(),
            key=lambda x: x[0]):  # sort to avoid non-determinism
    ...
    new_templates_dict = {
        tuple(tids): template
        for template, tids in inverse_templates_dict.items()
    }

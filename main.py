...
import time
import random
from GLIMPSE.GLIMPSE import GLIMPSEParser
...
if __name__ == "__main__":
    ...
    print(
        f"Total query time: {parser.wait_query_time} seconds, {parser.query_count} queries"
    )
    print(
        f"Avg. query time: {parser.wait_query_time/(parser.query_count+0.00001)} seconds"
    )
    print(
        f"Total pure match time: {total_time - parser.wait_query_time} seconds")

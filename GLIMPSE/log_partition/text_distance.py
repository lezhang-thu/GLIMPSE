...
def calculate_jaccard_one_to_many(str_anchor, text_list):
    ...
    similarity_list = [
        len(words_anchor & i) / len(words_anchor | i) for i in words_list
    ]
...
def calculate_jaccard_one_to_many_mask(str_anchor, text_list):
    ...
    words_counter = Counter(
        list(words_anchor) + [word for words in words_list for word in words])
...
def calculate_jaccard_self_loop(text_list):
    all_similarities = [
        calculate_jaccard_one_to_many(text_list[i], text_list[i + 1:])
        for i in range(len(text_list) - 1)
    ]
    all_similarities = [
        item for sublist in all_similarities for item in sublist
    ]
...
def calculate_jaccard_and_diff_self_all_comp(text_list, lamb=0.5):
    ...
    mean_div = sum([
        abs(x - y) for x in all_similarities for y in all_similarities
    ]) / (len(all_similarities)**2)
...
def calculate_jaccard_and_diff_self_loop_all_first(text_list, lamb=0.5):
    if len(text_list) == 0 or len(text_list) == 1:
        return 0
    all_similarities = calculate_jaccard_one_to_many(text_list[0],
                                                     text_list[1:])
    # mean_div = sum([(x - y) ** 2 for x in all_similarities for y in all_similarities]) / (len(all_similarities)**2)
    mean_div = sum([
        abs(x - y) for x in all_similarities for y in all_similarities

...
benchmark_settings = {
    ...
    "Thunderbird": {
        "log_file":
            "Thunderbird/Thunderbird_full.log",
        "log_format":
    ...
    "HiBench": {
        ...
        "regex": [
            r"hdfs://[a-zA-Z0-9._-]+(/[a-zA-Z0-9._/-]*)*",
...
def common_args():
    ...
    parser.add_argument('--add_regex',
                        type=str,
                        default="before",
                        choices=["add", "no", "before"])
    parser.add_argument('--regex', type=str, default="")
    parser.add_argument('--llm', type=str, default="DeepSeek-V3.2")
...
def parameter_args(parser):
    ...
    parser.add_argument('--sample_method',
                        type=str,
                        default="lcu_sampling",
                        choices=["lcu_sampling"])
    parser.add_argument('--sample_size',
                        type=int,
                        default=3,
                        help="parameter 1")
    parser.add_argument('--sample_min_similarity',
                        type=float,
                        default=0.5,
                        help="parameter 2")
    parser.add_argument('--lcu_lamb',
                        type=float,
                        default=0.6,
                        help="parameter 3")
    parser.add_argument('--lcu_sample_size',
                        type=int,
                        default=3,
                        help="parameter 4")
    parser.add_argument('--sample_size_auto',
                        type=str,
                        default="auto",
                        choices=["fixed", "auto"])
    ...
    parser.add_argument('--template_refine',
                        type=str,
                        default="TempDB",
                        choices=["No", "TempDB"])
    ...
    parser.add_argument('--prompt',
                        ...
                        default="VarExam",
                        choices=[
                            "VarOnly", "VarExam", "NoPE", "NoOutputConstraint",
                            "NoAdvice", "SimpleRequirements"
                        ])
...
def load_args():
    ...
    args.model = LLM_BASE_MAPPING[
        args.llm][0] if args.model == "" else args.model
    args.base_url = LLM_BASE_MAPPING[
        args.llm][1] if args.base_url == "" else args.base_url
    args.api_key = LLM_BASE_MAPPING[
        args.llm][2] if args.api_key == "" else args.api_key
    args.llm_params = {
        "model": args.model,
    ...
    # output dir
    if args.prefix == "":
        args.prefix = "glimpse-parallel-conf-exp"
